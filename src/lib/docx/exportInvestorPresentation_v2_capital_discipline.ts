// Investor presentation Word document — capital discipline edition (v2).
//
// Fork of exportInvestorPresentation.ts (v1). All structural rules preserved;
// prose rewritten for institutional investor / capital-discipline framing.
//
...
export async function exportInvestorPresentationV2CapitalDiscipline(
  ...
  children.push(
    ...
    new Paragraph({
      ...
      children: [new TextRun({
        text: 'Institutional Investor Memorandum — Capital Discipline Edition',
  ...
  children.push(
    bodyText(`Capital preservation is the primary structural principle: the downside floor IRR of ${pct(ds.equityIRR)} establishes the return under stress before considering the upside case. Equity is protected by a property-sale floor independent of hotel operating performance.`),
    bodyText(`Realistic base case equity IRR is ${pct(real.equityIRR)} on a ${real.exitYear - 2026}-year hold at ${mul(real.totalMOIC)} MOIC; stabilised cash yield reaches ${pct(real.yieldStabilised)}.`),
    bodyText(`Total CAPEX of ${eur(km.totalCapex)} funded by ${eur(km.loanAmount)} senior debt and ${eur(km.equityRequired)} equity; co-investor tranche is ${eur(coInvestorEquity)}. Debt service coverage at stabilisation is ${mul(real.stabilisedYear?.dscr ?? km.stabilisedDSCR)}×, against a 1.25× covenant floor.`),
    bodyText(`Three independent return levers: (1) annual cash yield during the hold, (2) MOIC at EBITDA-exit, (3) property-sale floor at ${pct(real.equityIRRPropertySale)} IRR — each positive under a disciplined stress scenario.`),
  );
  ...
  children.push(
    bodyText('Cyclades luxury hospitality demand is structurally undersupplied. The competitive set across Paros, Sifnos, and Folegandros trades at premium ADR multiples to the Attica baseline, with occupancy in the established luxury tier remaining robust across multiple seasons — validating pricing power before considering Antiparos specifically.'),
    bodyText('Supply response is structurally constrained: Antiparos zoning caps building coefficient on coastal plots; new hospitality licences require environmental studies, municipal approval, and coastal setback compliance typically exceeding three years — these are not cyclical barriers.'),
    bodyText(`Villa Lev operated at ${eur(a.revenueRealistic.villaADR)} ADR with ${a.revenueRealistic.villaBaseNights} bookable nights at stabilisation, consistent with the Cyclades luxury competitive set.`),
    bodyText(`Break-even is ${Math.round(km.breakEvenNights)} nights; the Realistic base case provides ${km.bufferToBreakEven >= 1 ? Math.round(km.bufferToBreakEven) + ' nights' : pct(km.bufferToBreakEven)} of headroom above break-even — a disciplined margin of safety for debt-service coverage.`),
  );
  ...
  children.push(
    ...
    bodyText(`Co-investor capital enters at construction commencement (${phase2Year}), not at acquisition — pre-construction risk (site, planning) is borne by the sponsor, structuring co-investor entry at the point where primary development risks have been resolved.`),
    bodyText(`Total CAPEX of ${eur(km.totalCapex)} covers hard construction ${eur(m.capex.portfolioTotal ?? 0)} and acquisition plus legal ${eur(m.capex.acquisitionLegal ?? 0)}; FF&E, soft costs, and contingency included within the envelope. LTV at construction cost is ${pct(km.ltv)}.`),
  );
  ...
  children.push(bodyText('Tranche schedule: sponsor equity at acquisition; co-investor equity at construction commencement; loan drawdown aligned to construction milestones with independent quantity surveyor sign-off at each stage.'));
  ...
  children.push(
    bodyText(`Return structured across three layers, in order of capital-preservation priority: (1) DSCR-covered annual cash yield during the ${real.exitYear - 2026}-year hold, (2) ${mul(real.totalMOIC)} MOIC at EBITDA-exit, (3) property-sale floor at ${pct(real.equityIRRPropertySale)} IRR realisable independently of hotel operating value.`),
    bodyText(`Realistic base case equity IRR is ${pct(real.equityIRR)}; downside scenario produces ${pct(ds.equityIRR)} IRR — confirming a positive return floor under a materially adverse revenue assumption. DSCR covenant headroom at stabilisation: ${pct(real.dscrCovenantHeadroom)} above the 1.25× floor.`),
    bodyText(`Terminal equity value of ${eur(real.terminalEquityValue)} reflects a ${real.exitEbitdaMultiple}× EBITDA exit; stabilised yield on committed equity is ${pct(real.yieldStabilised)}.`),
    bodyText(`Equity payback in ${real.equityPaybackYears !== null ? real.equityPaybackYears + ' years' : 'the hold period'}; cumulative yield at exit is ${pct(real.cumulativeYieldFinal)}. DSCR covenant headroom of ${pct(real.dscrCovenantHeadroom)} provides an early-warning buffer before any equity impairment event.`),
  );
  ...
  children.push(
    bodyText(`DSCR at stabilisation of ${mul(real.stabilisedYear?.dscr ?? km.stabilisedDSCR)}× against the 1.25× covenant floor represents ${pct(real.dscrCovenantHeadroom)} of headroom — a contractual early-warning mechanism giving the lender and co-investors advance notice of any operating shortfall before a covenant breach.`),
  ...
  children.push(
    bodyText(`The table below shows projected revenue, EBITDA, and DSCR across the ramp phase and stabilised year. Stabilised DSCR of ${mul(real.stabilisedYear?.dscr ?? km.stabilisedDSCR)}× is the lender anchor. Figures are forward-looking model outputs, not audited actuals.`),
    bodyText(`Stabilised revenue of ${eur(real.stabilisedYear?.totalRevenue ?? km.stabilisedRevenue)} produces EBITDA of ${eur(real.stabilisedYear?.ebitda ?? km.stabilisedEBITDA)} at a ${pct((real.stabilisedYear?.ebitda ?? km.stabilisedEBITDA) / ((real.stabilisedYear?.totalRevenue ?? km.stabilisedRevenue) || 1))} margin; the DSCR ramp from year 1 to stabilisation shows the debt-service loading profile an institutional lender would underwrite.`),
  );
  ...
  children.push(
    bodyText(`Equity IRR sensitivity: the downside scenario produces ${pct(ds.equityIRR)} equity IRR under a materially adverse combined shock of reduced occupancy and lower ADR — confirming capital preservation under stress.`),
    bodyText(`Asset coverage under stress: stressed collateral value of ${eur(m.collateral.stress.value)} at ${mul(m.collateral.stress.coverage)}× coverage confirms lender coverage; the ${eur(m.collateral.market.value - km.loanAmount)} equity cushion above the loan balance is intact at market value.`),
    bodyText(`DSCR covenant headroom of ${pct(real.dscrCovenantHeadroom)} above the 1.25× floor provides a contractual buffer before a covenant event; LTV of ${pct(km.ltv)} at close is conservative against the ${eur(m.collateral.market.value)} independent appraisal.`),
  );
  ...
  children.push(
    bodyText(`Waterfall mechanics: distributions pari-passu between sponsor and co-investor from construction commencement; no promote, no preferred return above the pari-passu rate. Co-investor has full tag-along rights at exit.`),
    bodyText(`Per-LP economics: co-investor equity of ${eur(coInvestorEquity)} participates in ${km.equityRequired > 0 ? pct(coInvestorEquity / km.equityRequired) : 'N/A'} of the equity pool on a pari-passu basis; total project distributable is ${eur(capResult.totalProjectDistributable)}; total distributed is ${eur(capResult.totalDistributed)}.`),
    bodyText('Construction-phase risk is borne exclusively by the sponsor; co-investor enters at the commencement milestone, after site acquisition and planning risk have been resolved.'),
    bodyText('The table below shows per-stakeholder IRR, MOIC, and payback year; the consolidated distribution schedule shows annual and cumulative cash flows from which individual returns are derived.'),
  );
  ...
  children.push(
    bodyText(`Hotel exit: a ${real.exitEbitdaMultiple}× EBITDA multiple in ${real.exitYear} produces terminal asset value of ${eur(real.terminalAssetValue)} before debt repayment; terminal equity value is ${eur(real.terminalEquityValue)}.`),
    bodyText('Named buyer universe: Greek and European family offices with island hospitality mandates; institutional operators expanding Aegean luxury portfolios; Cyclades-focused real estate funds. Exit timing risk is mitigated by the property-sale floor below.'),
    bodyText(`Property-sale floor: exit at €${real.exitValuationPerM2.toLocaleString()}/m² produces ${pct(real.equityIRRPropertySale)} equity IRR — fully independent of hotel EBITDA. Land scarcity in Antiparos supports the asset-value-per-key floor independent of operating performance.`),
  );
  ...
  children.push(
    bodyText('Quarterly reporting covenant: co-investor is entitled to receive quarterly management accounts, occupancy and ADR reports, and DSCR certification within 30 days of each quarter end. Annual audited accounts within 90 days of financial year end.'),
    bodyText('Audit rights: co-investor may commission an independent audit at any time with 15 business days notice; costs shared pro-rata with sponsor above one audit per calendar year.'),
    bodyText(`Due diligence checklist: title search and cadastral registration confirmation; planning permit and coastal setback compliance; independent construction cost QS report; environmental study approval; financing term sheet from ${financingPathLabel(a.financingPath)} lender; tax clearance certificate; corporate constitutional documents.`),
  );
